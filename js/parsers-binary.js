...
function parsePESymbols(view, e_lfanew) {
    ...
    try {
        ...
        const exportRVA =
            numDirs > 0 && dataDir + 40 <= view.byteLength ? view.getUint32(dataDir, true) : 0;
        const importRVA =
            numDirs > 1 && dataDir + 40 <= view.byteLength ? view.getUint32(dataDir + 8, true) : 0;
        ...
        if (importRVA) {
            ...
            if (importOffset) {
                ...
                while (safety++ < 64) {
                    ...
                    const thunkRVA =
                        view.getUint32(descOffset + 16, true) ||
                        view.getUint32(descOffset + 0, true);
                    ...
                    while (thunkOff && thunkSafety++ < 512) {
                        const thunkData = is64
                            ? view.getBigUint64(thunkOff, true)
                            : BigInt(view.getUint32(thunkOff, true));
                        if (thunkData === 0n) break;
                        const isOrdinal =
                            (thunkData & (is64 ? 0x8000000000000000n : 0x80000000n)) !== 0n;
                        if (isOrdinal) {
                            ...
                            const hintNameRVA = Number(
                                thunkData & (is64 ? 0x7fffffffn : 0x7fffffffn)
                            );
...
function parsePEImports(view, e_lfanew) {
    ...
    try {
        ...
        while (safety++ < 50) {
            if (
                view.getUint32(descOffset, true) === 0 &&
                view.getUint32(descOffset + 12, true) === 0
            )
                break;
            ...
            const thunkRVA =
                view.getUint32(descOffset, true) || view.getUint32(descOffset + 16, true);
...
function parseELF(view) {
    try {
        ...
        return {
            Arch: map[machine] || machine.toString(16),
...
function parseELFSections(view) {
    try {
        ...
        if (view.getUint32(shStrOff, isLE) === 0x6fffffff || view.getUint32(shStrOff, isLE) === 0) {
            ...
            const shstrOffset = is64
                ? Number(view.getBigUint64(shStrOff + 24, isLE))
                : view.getUint32(shStrOff + 16, isLE);
            const shstrSize = is64
                ? Number(view.getBigUint64(shStrOff + 32, isLE))
                : view.getUint32(shStrOff + 20, isLE);
            strTab = new Uint8Array(
                view.buffer,
                shstrOffset,
                Math.min(shstrSize, view.byteLength - shstrOffset)
            );
        }
        ...
        for (let i = 0; i < e_shnum; i++) {
            ...
            const flags = is64
                ? Number(view.getBigUint64(off + 8, isLE))
                : view.getUint32(off + 8, isLE);
            const addr = is64
                ? Number(view.getBigUint64(off + 16, isLE))
                : view.getUint32(off + 12, isLE);
            const shOffset = is64
                ? Number(view.getBigUint64(off + 24, isLE))
                : view.getUint32(off + 16, isLE);
            const shSize = is64
                ? Number(view.getBigUint64(off + 32, isLE))
                : view.getUint32(off + 20, isLE);
...
function parseELFSymbols(view) {
    try {
        ...
        for (let i = 0; i < e_shnum; i++) {
            ...
            const entsize = is64
                ? Number(view.getBigUint64(off + 56, isLE))
                : view.getUint32(off + 36, isLE);
        ...
        const getStrTable = (idx) => {
            ...
            return new Uint8Array(
                view.buffer,
                sec.offset,
                Math.min(sec.size, view.byteLength - sec.offset)
            );
        };
...
function parseELFImports(view) {
    try {
        ...
        const e_phoff = is64 ? Number(view.getBigUint64(32, isLE)) : view.getUint32(28, isLE);
        ...
        let dynSafety = 0;
        while (
            offset + dynEntSize <= view.byteLength &&
            offset < dynamicOff + dynamicSize &&
            dynSafety < 2000
        ) {
            const tag = is64
            ...
            if (tag === 10) _dynStrSize = val; // DT_STRSZ
        ...
        if (dynSymOff && dynSymOff < view.byteLength && strTableReady) {
            ...
            let safety = 0;
            while (
                (symLimit ? symOffset < symLimit : safety < 800) &&
                symOffset + dynSymEntSize <= view.byteLength
            ) {
        ...
        if (importedSyms.length > 0)
            result['Imported Functions (Undefined Symbols)'] = importedSyms;
...
function parseMachO(view) {
    ...
    const parseSingleMachO = (baseOffset, isLittle, magicVal) => {
        ...
        if (symtabInfo && symtabInfo.symoff + (is64 ? 16 : 12) <= view.byteLength) {
            ...
            const strTab = new Uint8Array(
                view.buffer,
                symtabInfo.stroff,
                Math.max(0, strTabEnd - symtabInfo.stroff)
            );
            const readStrx = (off) => {
            ...
            for (let i = 0; i < maxSyms; i++) {
                ...
                const n_value = is64
                    ? Number(view.getBigUint64(base + 8, isLittle))
                    : view.getUint32(base + 8, isLittle);
    ...
    if (isFat) {
        ...
        if (!firstArchOffset || firstArchOffset >= view.byteLength)
            return { metadata: { 'Mach-O': `Fat binary (${nfat} arch)` }, sections: [] };
        ...
        if (innerMagicBE === MAGIC.MH_MAGIC || innerMagicBE === MAGIC.MH_MAGIC_64)
            innerLittle = false;
        else if (innerMagicBE === MAGIC.MH_CIGAM || innerMagicBE === MAGIC.MH_CIGAM_64)
            innerLittle = true;
        else if (innerMagicLE === MAGIC.MH_MAGIC || innerMagicLE === MAGIC.MH_MAGIC_64) {
            innerLittle = true;
            innerMagic = innerMagicLE;
        } else if (innerMagicLE === MAGIC.MH_CIGAM || innerMagicLE === MAGIC.MH_CIGAM_64) {
            innerLittle = false;
            innerMagic = innerMagicLE;
        }
